...
public class GetComments extends AsyncTask<String, Void, Comments[]> {

	private final String URL = Keys.BASE_URL + "offer/comments/";
	private ProgressDialog dialog;
	private OnProcessCompleteListener callback;
	private Context context;

	public GetComments(Context context, OnProcessCompleteListener cb) {
	...
	protected Comments[] doInBackground(String... params) {
		...
		if (responseJSON != null && responseJSON.length() > 1) {
			...
			gson = gb.create();
			try {
				obj = gson.fromJson(responseJSON, Comments[].class);
