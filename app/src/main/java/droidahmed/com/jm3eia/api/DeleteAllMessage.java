...
public class DeleteAllMessage extends AsyncTask<String, Void, ResultWS> {

	private final static String URL = Keys.BASE_URL + "message/delete_all";

	private ProgressDialog dialog;
	private OnProcessCompleteListener callback;
	private Context context;

	public DeleteAllMessage(Context context, OnProcessCompleteListener cb) {
	...
	protected void onPreExecute() {
		this.dialog.setMessage(context.getResources().getString(
				R.string.loading_remove_messages));
		this.dialog.setCancelable(false);
		this.dialog.show();
	}

	@Override
	protected ResultWS doInBackground(String... params) {
		...
		if (responseJSON != null && responseJSON.length() > 1) {
			...
			gson = gb.create();
			try {
				obj = gson.fromJson(responseJSON, ResultWS.class);
	...
	@Override
	protected void onPostExecute(ResultWS result) {
		if (dialog.isShowing()) {
			dialog.dismiss();
		}
		if (result != null) {
