...
public class ShowOffer extends AsyncTask<String, Void, Offer[]> {

	private final String URL = Keys.BASE_URL + "offer_show";
	private ProgressDialog dialog;
	private OnProcessCompleteListener callback;
	private Context context;

	public ShowOffer(Context context, OnProcessCompleteListener cb) {
	...
	protected Offer[] doInBackground(String... params) {
		...
		if (responseJSON != null && responseJSON.length() > 1) {
			...
			gson = gb.create();
			try {
				obj = gson.fromJson(responseJSON, Offer[].class);
